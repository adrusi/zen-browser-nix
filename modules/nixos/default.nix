...
let
  ...
  organisationInfo = ''
    When this option is in use, Zen will inform you that "your browser
...
in
{
  options.programs.zen-browser = {
    enable = lib.mkEnableOption "the Zen web browser";
    ...
    wrapperConfig = lib.mkOption {
      ...
      default = { };
      description = "Arguments to pass to Zen wrapper";
    };
    ...
    preferencesStatus = lib.mkOption {
      ...
      default = "locked";
      description = ''
        The status of `zen-browser.preferences`.
  ...
  config =
    ...
    lib.mkIf cfg.enable {
      warnings = forEachEnabledNmh (
        ...
        + "please add `${v.package.pname}` to `programs.zen-browser.nativeMessagingHosts.packages` instead."
      );
      programs.zen-browser.nativeMessagingHosts.packages = forEachEnabledNmh (_: v: v.package);
